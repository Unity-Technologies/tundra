...
static bool AreNodeFileAndGlobSignaturesStillValid(RuntimeNode* node, ThreadState* thread_state, const char** outOfDateSignaturePath)
{
    auto VerifyNodeGlobSignatures = [=]() -> bool {
        for (const Frozen::DagGlobSignature &sig : node->m_DagNode->m_GlobSignatures)
        {
            ...
            {
                *outOfDateSignaturePath = sig.m_Path.Get();
                return false;
            }
        }
        return true;
...
static NodeBuildResult::Enum ExecuteNode(BuildQueue* queue, RuntimeNode* node, Mutex *queue_lock, ThreadState* thread_state, StatCache* stat_cache, const Frozen::DagDerived* dagDerived, const char** out_outOfDateSignaturePath)
{
    CheckDoesNotHaveLock(&queue->m_Lock);
    ...
    if (runActionResult == NodeBuildResult::kRanSuccesfully && !AreNodeFileAndGlobSignaturesStillValid(node,thread_state, out_outOfDateSignaturePath))
...
static void ProcessNode(BuildQueue *queue, ThreadState *thread_state, RuntimeNode *node, Mutex *queue_lock)
{
    ...
    {
        const char* outOfDateSignaturePath = nullptr;
        MutexUnlock(queue_lock);
        NodeBuildResult::Enum nodeBuildResult = ExecuteNode(queue, node, queue_lock, thread_state, thread_state->m_Queue->m_Config.m_StatCache, queue->m_Config.m_DagDerived, &outOfDateSignaturePath);
        MutexLock(queue_lock);
        ...
        switch (node->m_BuildResult = nodeBuildResult)
        {
            ...
            case NodeBuildResult::kUpToDataButDependeesRequireFrontendRerun:
                if (queue->m_FinalBuildResult == BuildResult::kOk)
                {
                    ...
                    queue->m_OutOfDateSignaturePath = outOfDateSignaturePath;
