...
static NodeBuildResult::Enum ExecuteNode(BuildQueue* queue, RuntimeNode* node, Mutex *queue_lock, ThreadState* thread_state, StatCache* stat_cache, const Frozen::DagDerived* dagDerived)
{
    ...
    if (IsNodeCacheableByLeafInputsAndCachingEnabled(queue,node))
    {
        ...
        if (node->m_BuiltNode)
        {
            bool wasSuccessfulWithGuaranteedCorrectInputSignature = node->m_BuiltNode->m_Result == Frozen::BuiltNodeResult::kRanSuccessfullyWithGuaranteedCorrectInputSignature;
            bool stillTheSame = wasSuccessfulWithGuaranteedCorrectInputSignature && node->m_BuiltNode->m_LeafInputSignature == node->m_CurrentLeafInputSignature->digest;
            if (!stillTheSame && !VerifyAllVersionedFilesIncludedByGeneratedHeaderFilesWereAlreadyPartOfTheLeafInputs(queue, thread_state, node, dagDerived))
                return NodeBuildResult::kRanFailed;
        }
    }

    // Compute timestamps and record the latests file modification date we find for any input file not generated by the graph.
    ...
    if (latestTimestampSeenForNonGeneratedInputFile >= FileSystem::g_LastSeenFileSystemTime)
    {
        ...
        else if (latestTimestampSeenForNonGeneratedInputFile > fileSystemTimeNow)
        {
            ...
            // lock up tundra for a very long time. Instead we record this specific state and flag input siganture as "might be incorrect".
